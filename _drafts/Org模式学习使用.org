#+TITLE: Org 模式学习使用
本文是学习使用 Org 模式的练习文档，原文来自于 csqlwy 的文章 [[http://www.cnblogs.com/qlwy/archive/2012/06/15/2551034.html][神器中的神器 org-mode 之入门篇]]
...
* 3. 安装及简单配置
Emacs23 之后已经默认包含有 Org-mode 模式了，但是网上都说新版的比较好，我于是就下了个目前最新的稳定版，解压到 site-lisp 目录下后，在 .emacs 中配置如下：
#+BEGIN_SRC emacs-lisp 
;; 配置 Org-mode
(setq load-path (cons "E:/emacs-23.4/site-lisp/org-7.8.11/lisp" load-path))
(require 'org-install)
#+END_SRC
现在如果你新建一个 .org 格式的文件，如果在 minibuffer 中显示 Org 的字样，就意味着你已经成功了。
...
#锚点<<target>> 这里我把锚点设置到这一部分开始处，大家可以点击下面效果中两个连接试试效果，如下方式可以设置到 target 的链接：
#+BEGIN_EXAMPLE
