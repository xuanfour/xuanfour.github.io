...
- [Mac 安装使用](#mac-安装使用)
    ...
    - [brew](#brew)
        - [安装 brew](#安装-brew)
        - [更新 Homebrew](#更新-homebrew)
        - [更新包](#更新包)
        - [更新需要更新的包](#更新需要更新的包)
        - [清理旧版本](#清理旧版本)
        - [锁定不想更新的包](#锁定不想更新的包)
        - [其他几个常用命令](#其他几个常用命令)
        - [修改 brew 国内源](#修改-brew-国内源)
            - [步骤](#步骤)
            - [替换 Homebrew Bottles 源](#替换-homebrew-bottles-源)
            - [切换回官方源](#切换回官方源)
        - [Homebrew Cask 更换国内源](#homebrew-cask-更换国内源)
            - [替换为 USTC 镜像：](#替换为-ustc-镜像：)
            - [重置为官方地址：](#重置为官方地址：)
        - [使用指定本地安装包](#使用指定本地安装包)
    - [安装 frp](#安装-frp)
    - [安装 rbenv ruby rails](#安装-rbenv-ruby-rails)
- [安装 rbenv](#安装-rbenv)
- [确认 rbenv 的版本](#确认-rbenv-的版本)
- [更新 rbenv](#更新-rbenv)
- [配置 rbenv](#配置-rbenv)
- [然后把下面的代码放到 ~/.bash_profile 里](#然后把下面的代码放到-bashprofile-里)
- [注意：Unubtu 请放到 ~/.bashrc 里, zsh 用户是 ~/.zshrc](#注意：unubtu-请放到-bashrc-里-zsh-用户是-zshrc)
- [或者重开一个终端](#或者重开一个终端)
- [安装插件 ruby-build](#安装插件-ruby-build)
- [通常安装 ruby 最新版需要安装 ruby-build 插件用来编译安装 ruby](#通常安装-ruby-最新版需要安装-ruby-build-插件用来编译安装-ruby)
- [普通用户不能监听 1024 以下端口，需要 sudo 授权](#普通用户不能监听-1024-以下端口需要-sudo-授权)
- [更新插件 ruby-build](#更新插件-ruby-build)
- [安装插件 rbenv-gemset](#安装插件-rbenv-gemset)
- [用来管理 gemset, 可选, 因为有 bundler 也没什么必要](#用来管理-gemset-可选-因为有-bundler-也没什么必要)
- [安装插件 rbenv-update](#安装插件-rbenv-update)
- [通过 rbenv update 命令来更新 rbenv 以及所有插件, 推荐](#通过-rbenv-update-命令来更新-rbenv-以及所有插件-推荐)
- [安装插件 rbenv-china-mirror](#安装插件-rbenv-china-mirror)
- [使用 Ruby China 的镜像安装 Ruby, 国内用户推荐](#使用-ruby-china-的镜像安装-ruby-国内用户推荐)
- [安装 ruby](#安装-ruby)
- [解决无法在 irb 中输入中文的方法](#解决无法在-irb-中输入中文的方法)
- [列出版本](#列出版本)
- [设置版本](#设置版本)
- [rbenv 支持以下三种 Ruby 版本的环境管理：](#rbenv-支持以下三种-ruby-版本的环境管理：)
- [global：设置全局的 Ruby 版本，换句话说，所有的 shell 都将使用该 Ruby 版本。](#global：设置全局的-ruby-版本换句话说所有的-shell-都将使用该-ruby-版本)
- [local：为本地的一个特定项目设置 Ruby 版本，注意这将覆盖全局设置。](#local：为本地的一个特定项目设置-ruby-版本注意这将覆盖全局设置)
- [shell：针对 shell 设置 Ruby 版本，该设置将覆盖 global 和 local 设置。](#shell：针对-shell-设置-ruby-版本该设置将覆盖-global-和-local-设置)
- [rbenv shell 1.9.3-p392       # 当前的 shell 使用 1.9.3-p392, 会设置一个 `RBENV_VERSION` 环境变量](#rbenv-shell-193-p392--------当前的-shell-使用-193-p392-会设置一个-rbenvversion-环境变量)
- [rbenv local jruby-1.7.3      # 当前目录使用 jruby-1.7.3, 会生成一个 `.rbenv-version` 文件](#rbenv-local-jruby-173-------当前目录使用-jruby-173-会生成一个-rbenv-version-文件)
- [ruby 配置](#ruby-配置)
    - [安装 pyenv 和 python](#安装-pyenv-和-python)
        - [安装 zlib 依赖](#安装-zlib-依赖)
        - [pyenv 使用指南](#pyenv-使用指南)
        - [pyenv 的其他操作](#pyenv-的其他操作)
        - [pyenv 命令](#pyenv-命令)
        - [pyenv-virtualenv 安装](#pyenv-virtualenv-安装)
            - [安装](#安装)
            - [使用](#使用)
    - [安装 node](#安装-node)
        - [安装 nvm](#安装-nvm)
...
# 更改本机版本，重启不会造成再次更改
# 设置 Global Python 为 2.7, 备注：尽量不要把 Py3 设置为全局，否则由于 Homebrew 本身有一些应用是依赖于 Py2 的，设置为 Py2 容易出现一些奇怪的问题。
pyenv global 2.7.15
# 安装常用包
...
| pyenv global          | 告诉全局环境使用某个版本，为了不破坏系统环境，不建议使用 global 设置全局版本     |
